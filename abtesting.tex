...
\thispagestyle{empty}
\epigraph{I am a big believer in reason and facts and evidence and science and feedback - everything that allows you to do what you do} {Barack Obama. \cite{Christian2012}}
...
Businesses such as Netflix \cite{Liu2014}, Amazon \cite[p. 147]{Kohavi2009}, Google \cite{Christian2012} and others have successfully increased \textit{return on investment} or ROI of their products and services. Instead of trusting the \textit{Hippo} or Highest paid persons' opinion, design choices are made by running experiments.
...
\section{Methods}
The freemium business model is viable like never before due to a confluence of growth of mobile computing (Figure \ref{mobile}) and global app stores. Stores such as Apple \textit{AppStore} (75 billion downloads by September 2014 \cite{Perez2014}) and \textit{Google Play} (50 billion downloads \cite{Victor2013}) have given global reach with a marginal cost of production. Combined with a price point of \$0 the inhibitors for explosive growth are very low. Massive scale allows to find players who are willing to pay for premium services within the game or service. \cite{Seufert2014} Mobility allows users to make purchases at any time of day at any place. In order to reach those willing to pay more for the product than if it was a premium game, the experience needs to be built from the ground up to be freemium. It cannot be added on and requires skilled labor. Do you have a team that is able to deliver? If the decision is made to go freemium, insight into user behavior in the game context is cardinal. A part of this insight is studying collected user data.\cite{Seufert2014}

\begin{figure}[htb]
\centering \includegraphics[height=10cm]{diagrams/digital_devices}
\caption{Streamgraph: Adoption of mobile phones takes over pc market. 2012 mobile device adoption overtook the pc market. Data from \cite{Heggenstuen2013}\label{mobile}}
\end{figure}

A simple A/B test consists of the control group and the variant, which are exposed to unique sets of users. The goal could be to improve retention and or conversion. The setup is quite similar to clinical trials, where the goal may be to measure the effectiveness of a treatment for example.
...
When trying to improve design of a game, asking the question that needs to be answered is vital, because it brings focus to the rest of the process, just like when the scientific method is applied elsewhere. The an answer is attempted through experimentation. In order to approve.

\subsection{Metrics}
Actionable freemium metrics fall into retention, engagement, monetization or virality categories. Retention is a time-based metric of product use. High retention is at the heart of every successful freemium product. If users don't play the game, they cannot be expected to make purchases. \cite{Seufert2014}

\subsubsection{Retention}
The simplest retention metrics are \textit{daily new users} (\emph{DNU}) or how many new users played the game on a given day and  \textit{daily active users} (\emph{DAU}) how many total users played the game on that day. Other common metrics are \emph{D1}, \emph{D3}, \emph{D7}, \emph{D14} and \emph{D30} where \emph{D14} for example means ''what percentage of players was seen 14 days after first using it''.\cite{Seufert2014} Note the lag of data for days where not enough time has passed to calculate the metric.

\begin{equation}
  Da = \frac{ru_{today}}{dnu_{a}} * 100
\end{equation}
$Da$-retention in \% where $ru_{today}$ is returning users today who first used the product yesterday and $dnu_{a}$ is $a$ days from today. In figure \ref{retention-table}, you can see the metrics in a single column. One downside of retention metrics is that they require large volumes of users to settle. If the number of users is small, outliers may affect the data making it look false. \cite{Seufert2014} In order to better understand product change effects on retention, they are best measured so that the current days activity does not affect that days retention metrics. A common way to visualize retention is to make a timeseries with a value of $D1$, $D3$, $D7$ and $D30$ in separate timeseries (Figure \ref{retention-plot}).

\begin{table}[htb]
\caption{Example retention metrics for a given day\cite{Seufert2014}\label{retention-table}}
\begin{center}
\fbox{
\begin{tabular}{l|c}
 &  2013-01-01\\ \hline
DNU & 4\\ \hline
D1 retention & 75\% \\ \hline
D2 retention & 50\% \\ \hline
D3 retention & 25\% \\ \hline
D4 retention & 25\%
\end{tabular}
}
\end{center}
...
\begin{figure}[htb]
\centering \includegraphics[height=10cm]{diagrams/retention}
\caption{Retrospective retention plot \cite{Seufert2014}\label{retention-plot}}
\end{figure}

Churn is a metric for measuring users leaving the game. It's tricky to know after how many days it's likely that a user never returns to the game. Therefore it's good to use a threshold like 80% of users don't return after being unseen for $n$ days.\cite{Seufert2014}


\subsubsection{Monetization}
''Where's the **cking money Lebowski?'' \textit{NPU} is the non paying user who don't contribute to revenue, but are the heart of the freemium game.  Revenue in freemium games usually comes from advertising and in app purchases or \textit{IAP}s.\cite{Seufert2014} Advertising should be treated separately, as it's a complex subject. We will concentrate only on \textit{IAP}s.

The \textit{conversion rate} $C_{r}$ is
\begin{equation}
  C_{r} = \frac{n_{paying}}{n_{total}}
\end{equation}

where $n_{paying}$ is the number of paying users and $n_{total}$ is the total number of users in the game. \textit{ARPU} or average revenue per user will tell you how much money, on average a user puts into the game. \textit{ARPPU} is the average revenue per paying user and will tell you how much money the paying user puts into the game. If the conversion rate is expected to be $5\%$, then it will be a small number.

In order to see how money spen fluctuates daily, \textit{ARPDAU} or average revenue per daily active user is a useful metric. The metric most affected by changes in prices and other monetization related parameters, is the daily \textit{ARPPU}. \cite{Seufert2014}

The product catalog histogram can tell you where the money comes from and how it is distributed. This should be over a sliding time period as otherwise it will not be sensitive to changes other than addition and removal. In order to understand the users behaviour when starting to spend money, \textit{first purchase distribution} can be extremely useful. It will tell what user bought first. \cite{Seufert2014}

\subsubsection{Engagement}
Engagement sheds light into how users interact with your game. The onboarding funnel gives information on first use of the application. Daily sessions and session length tell you how long and how many sessions on average the users have per day. Also the \textit{net promoter score} is a qualitative metric on how much players enjoy or loathe your product.

The onboarding funnel (figure \ref{onboarding-funnel}) is constructed by following a defined set of successive events per user and see where they abandon the game.
\begin{figure}[htb]
\centering \includegraphics[height=10cm]{diagrams/onboarding_funnel}
\caption{Example onboarding funnel \cite{Seufert2014}\label{onboarding-funnel}}
\end{figure}

Daily average session length is
\begin{equation}
Sl_{avg} = \frac{Sl_{all}}{S_{all}}
\end{equation}
and can vary wildly \cite{Seufert2013}. The daily median session length will give indication of how skewed the average is.

Daily average sessions is
\begin{equation}
  S_{avg} = \frac{S_{all}}{DAU}
\end{equation}

and will tell how often players return to the game per day.

The net promoter score or \textit{NPS} measures loyalty in customer relationships. The user, player or customer is asked one question ''How likely is it that you would recommend our company/product/service to a friend or colleague?'' The scale could be 0 to 10 for example. \cite{Reichheld2003} Those who answered below 6 are detractors, 7 and 8 are netural. Nine and ten are considered promoters \cite{Suefert2014} Neutral scores are scratched and the percentage of detractors is subtracted from percentage of promoters. Over 0 is good and over 50 unbelievably good. \cite{Seufert2014}
...
The \textit{k-factor} is the average number of users each user recruits to play the game.
\begin{equation}
  k_{factor} = \frac{UA_{virally}}{UA_{all}}
\end{equation}
and can be quite hard to calculate as the users aquired virally are difficult to track, and if not, the it's difficult to acquire the data. This number should be treated as an estimate. If the \textit{k-factor} is 1, it means linear growth, and for 2 and higher the growth is geometric. \cite{Seufert2014}

\subsubsection{Lifetime customer value}
In order to know how much a user is willing to spend on a game the \textit{lifetime customer value} or \textit{LTV} is calculated. It is all the future cash flows that a player is expected to bring in.
\begin{equation}
  LTV = \sum\limits_{t=0}^T\frac{(P_{t} - C_{t})r_{t}}{(1 + i)^t} - AC
\end{equation}
by Gupta et al. \cite{Gupta2006} where $P_{t}$ is the price paid by consumer at $time = t$, $C_{t}$ is the cost of servicing a customer at $time = t$. $i$ is the discount rate, $r_{t}$ is the probability that the user is active at $time = t$. $AC$ is the acquisition cost of the user. It is a prediction, but an important one. It sets a maximum on how much can be spent on user acquisition, or paying to gain users through product marketing.\cite{Seufert2014} Another way to calculate \emph{LTV} is using the \textit{Pareto NBD} method \cite{Schmittlein1987} which makes the prediction from irregular revenue streams.

Now that we have established some metrics to measure, the most common and easiest probably being retention and conversion, we can begin optimization using controlled experimentation.

\clearpage

\subsection{Controlled Experiment}
The A/B Test, a simple controlled experiment exposes two different versions of the game, or versions of game parts to unique sets of players. The intent usually is to improve conversion or retention, but these are other goals. The control group represents current game design. The variant is a game design variation. The control and the variant are exposed to different sets of players and player behaviour is observed and analyzed.
...
ssL\"ahdeviittaukset tulee tehd\"a huolellisesti ja johdonmukaisesti
