...
Businesses such as Netflix \cite{Liu2014}, Amazon \cite[p. 147]{Kohavi2009}, Google and others have successfully used A/B testing for making product design choices to increase ROI. Instead of trusting the \textit{Hippo} or Highest paid persons' opinion, fine tuning design choices is made by running experiments.

%%TODO: general stuff on ab testing, what its used for and where. then something on the gaming industry and then freemium. and how that ties in to
...
Could this process be automated by a tool so that the test could be run by the game development team as they see fit instead of hiring a professional to run and analyze the tests? Who are the users of such systems? What are their demands? What are the best practices for running A/B Tests for optimizing a freemium game experience?

The science behind testing is well know as are the methods of testing games, but can we automate test steps of deploying variants, running test, reaching confidence and deploying the winner to all users? Can a usable A/B Testing system be created so that it can safely be used by non professionals to improve freemium game conversion and retention. Moving from some testing by professionals, to ''Always be Testing'' \cite{Siroker2013} mantra supported by tooling.

The A/B Testing system is a confluence of the game team and their tools; the players and their devices and the environment that they are in. The design science paradigm is a good fit for a method to conduct research for such an A/B testing system.
...
\bibitem{times-funding-2012} Ashkenas, J., Ericson, M., Parlapiano, A., Willis, D., The 2012 Money Race: Compare the Candidates
